...
def get_psth(num, date, proc_dir, start_time, stop_time, timebin, n_stimuli=None):
    """
    ...
    logging.debug(f'{mwk_files[0]}, {dirs[0]}')
    print(len(mwk_files))
    print(len(dirs))
...
def combine_channels(proc_dir, num_channels=192):
    """
    ...
    #dirs = [_ for _ in dirs if any(date in _ for date in dates)]
    for d in dirs:
        ...
        if not os.path.isfile(os.path.join(proc_dir, d, filename)):
            ...
            print(f'{len(ch_files)} files in {d}')
...
def combine_sessions(dates, proc_dir, output_dir, normalize=False, save_format='h5'):
    ...
    for i, d in enumerate(dirs):
        ...
        if normalize:
            if date != prev_date:
                ...
                assert normalizer_p.shape[0] == 86  # 85 normalizers images + 1 gray image
                ...
                normalizer_p_no_grey = normalizer_p[images_no_grey,:,:,:] # Select all images except grey (#26),
                n_p = np.nanmean(normalizer_p_no_grey[:, :, t_cols, :], 2) # then mean 70-170 time bins
                assert n_p.shape[0] == 85
    ...
    print(f'Combining sessions - {psth_matrix.shape}')
    ...
    print(f'Trimming all NaN repetition columns - {psth_matrix.shape}')

    experiment_name = proc_dir.split('/monkeys/')[0].split('/')[-1]
    monkey_name = proc_dir.split('/monkeys/')[1].split('/')[0]

    # Save experiment PSTH
    filename = f'{dates[0]}.{monkey_name}.rsvp.{experiment_name}.experiment_psth.{save_format}'
    if not normalize:
        filename = f'{dates[0]}.{monkey_name}.rsvp.{experiment_name}.experiment_psth_raw.{save_format}'
    ...
    if normalize:
        ...
        print(f'Normalizer - {normalizer_matrix.shape}')
...
if __name__ == '__main__':
    combine_channels(proc_dir='/braintree/data2/active/users/sgouldin/projects/robustness_guy_d0_v7_chong/monkeys/pico/intanproc')
    combine_sessions(dates=['230724'],
                     proc_dir='/braintree/data2/active/users/sgouldin/projects/robustness_guy_d0_v7_chong/monkeys/pico/intanproc',
                     output_dir='/braintree/data2/active/users/sgouldin/projects/robustness_guy_d0_v7_chong/monkeys/pico/h5',
                     normalize=False,
                     save_format='h5')
